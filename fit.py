...
class FitRunner:
    ...
    def visualize(self, save=None):
        ...
        plt.hist(data_arr, bins=1000, alpha=0.5, density=True)
        calc = lambda x: (self.weight * self.dist_a(x) + (1 - self.weight) * self.dist_b(x))
...
if __name__ == '__main__':
    ...
    a2, b2 = 8, 12
    weight = 0.4
    arr = np.array([(np.random.gamma(a1, 1/b1) if np.random.uniform(0, 1) < weight else np.random.gamma(a2, 1/b2))  for _ in range(50000)] + [np.clip(np.random.normal(1.5, 0.2), 0, 20) for _ in range(5000)])
